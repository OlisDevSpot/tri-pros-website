...
export function AllAnalyticsPanel({ chip, from, to }: Props) {
  const trpc = useTRPC()
  const analyticsQuery = useQuery({
    ...trpc.leadSourcesRouter.getAggregateAnalytics.queryOptions({ from, to }),
  ...
  if (!analyticsQuery.data) {
    return <AggregateAnalyticsSkeleton />
  }
...
function AggregateAnalyticsSkeleton() {
  return (
