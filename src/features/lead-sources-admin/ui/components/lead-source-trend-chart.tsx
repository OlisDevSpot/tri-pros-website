...
export function LeadSourceTrendChart({ trend, bucket, chip }: Props) {
  const hasData = trend.length > 0 && trend.some(p => p.leads > 0 || p.meetings > 0 || p.signed > 0)
      ...
      {hasData
        ...
        : (
            <EmptyState
              ...
              description="Try a longer time range to see trend data."
            >
              <ActivityIcon size={48} className="text-muted-foreground/50" />
            </EmptyState>
          )}
    </div>
  )
