...
/**
 * Read-only readout of the dialer (JustCall) campaigns we've synced, plus the
 * Resync trigger. Campaigns are pools (no source column here) — the
 * source→campaign binding lives in the per-source policy table. Status uses a
 * quiet dot + label, not a primary badge, to keep the single primary moment for
 * "Live" sources in the policy table. see docs/ui-design-playbook.md
 */
    ...
    <Card className="h-full">
      <CardHeader className="flex flex-row items-start justify-between gap-4">
        <div className="flex flex-col gap-1">
          ...
          <CardDescription>Pulled from JustCall. Resync after adding a campaign there.</CardDescription>
        </div>
        <Button
          size="sm"
          variant="outline"
          ...
          : campaigns.length === 0
            ? (
                <p className="py-4 text-sm text-muted-foreground">
                  No campaigns synced yet. Resync to pull them from JustCall.
                </p>
              )
            : (
                <>
                      ...
                      {campaigns.map((campaign) => {
                        ...
                        return (
                          <TableRow key={campaign.id}>
                            <TableCell className="font-medium text-foreground">{campaign.providerCampaignName}</TableCell>
                            <TableCell>
                            ...
                            <TableCell>
                              <code translate="no" className="font-mono text-xs text-muted-foreground">{campaign.dialerMode}</code>
                            </TableCell>
                            <TableCell>
                              {on && n > 0
                                ? (
