...
export function ProjectStoryCard({ slide, index }: ProjectStoryCardProps) {
  ...
  const inner = (
    <article className="group/card grid grid-cols-1 lg:grid-cols-12 gap-0 bg-foreground/[0.02] border border-foreground/10 overflow-hidden">
        ...
        {slide.imageUrl
          ? (
              <Image
                ...
                className="object-cover transition-transform duration-[1100ms] ease-[cubic-bezier(0.16,1,0.3,1)] group-hover/card:scale-[1.04]"
              />
            )
          : (
              <div className="h-full w-full bg-gradient-to-br from-foreground/10 to-foreground/[0.02]" />
            )}
        <span
          aria-hidden
      ...
      <div className="relative lg:col-span-5 flex flex-col items-center text-center lg:items-start lg:text-left justify-center p-8 sm:p-10 lg:p-12 xl:p-14 gap-6 lg:gap-8">
        ...
        <div className="relative inline-flex items-center gap-3 text-[10px] font-medium uppercase tracking-[0.22em] text-muted-foreground">
          ...
          <span>{slide.kind === 'project' ? 'Project Story' : 'Client Voice'}</span>
        </div>
        ...
        {slide.href
          ? (
              <div className="relative pt-2">
                <span className="group inline-flex items-center gap-2 text-[11px] font-medium uppercase tracking-[0.2em] text-foreground">
                  <DrawnUnderline>View This Project</DrawnUnderline>
                  <ArrowUpRight className="size-3.5 transition-transform duration-200 group-hover/card:translate-x-0.5 group-hover/card:-translate-y-0.5" />
                </span>
              </div>
            )
          : null}
