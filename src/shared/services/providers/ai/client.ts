...
//   await aiClient.generateProjectSummary(proposalId, proposalFormValues)
//
// Never reach into provider internals — go through this client.
//
// NOTE (follow-up): generateProjectSummary currently persists to the DB inside
// the provider. A provider should be a leaf (call the model, return output) with
// persistence owned by ai.service.ts / the proposals DAL. Tracked for a deeper
// pass — see the providers boundary-refactor issues.
// ---------------------------------------------------------------------------
