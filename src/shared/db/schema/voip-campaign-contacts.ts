...
// Per-customer dialer campaign-participation record. ONE row per customer who
// has ever been pushed to the dialer (JustCall). This is the single home for ALL
// voip-campaigns per-customer state (renamed from `voip_contact_sync` 2026-06-04
// to reflect the expanded role). `customers` carries NO voipCampaign* fields —
// only the shared DNC fields, which both EPICs write.
//
// State model (perfect separation — the dialer owns lifecycle; we own membership):
//   - Enrolled now      = row exists AND unenrolled_at IS NULL.
//   - Unenroll          = dialerProvider.unenroll(campaignId, providerContactId) +
//                         set unenrolled_at + unenroll_reason
//                         ('graduated' | 'opted_out' | 'disqualified'). The row and
//                         provider_contact_id PERSIST so re-enroll reuses the same
//                         provider contact (clear unenrolled_at + reason, set a new
...
//                         reachable from BOTH the UI and a dialer webhook disposition.
//   - Which campaign     = RECORDED in `voip_campaign_id` (FK → voip_campaigns), NOT
//                         derived. A lead source owns MANY campaigns; a customer is
//                         enrolled in exactly ONE at a time. Enrollment/unenroll pass
//                         that campaign's `provider_campaign_id` to the dialer.
//                         Re-enroll may point at a different campaign (update the FK).
//
...
// unrelated reasons.
//
// `dial_attempts` is moved off `customers` (2026-06-04). The dialer fires no
