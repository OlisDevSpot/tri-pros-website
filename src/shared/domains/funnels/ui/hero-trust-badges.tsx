...
import { reviews, socials } from '@/shared/constants/company'
  ...
  return (
    <div className="flex flex-wrap items-center justify-center gap-2 @3xl:justify-start">
      <a
        ...
        aria-label={`Google reviews: ${google.rating.toFixed(1)} out of 5 from ${google.count} reviews (opens in a new tab)`}
      ...
      >
        ...
        <span className="text-foreground text-sm font-semibold tabular-nums">{google.rating.toFixed(1)}</span>
      </a>
      <a
        href={yelpHref}
        target="_blank"
        rel="noopener noreferrer"
        aria-label={`Yelp reviews: ${yelp.rating.toFixed(1)} out of 5 from ${yelp.count} reviews (opens in a new tab)`}
        className={`${pill} transition-colors hover:border-(--accent-ink)`}
      >
        <FaYelp className="text-foreground size-4" aria-hidden="true" />
        <span className="text-foreground text-sm font-semibold">Yelp</span>
        <span className="text-foreground text-sm font-semibold tabular-nums">{yelp.rating.toFixed(1)}</span>
      </a>
      <div className={pill}>
        <span className="text-foreground text-sm font-semibold">BBB</span>
        <span className="text-foreground text-sm font-semibold">{bbb.rating}</span>
      </div>
    </div>
  )
}
