...
This directory holds: schemas (`schemas/`), types (`types.ts`), enum constants and action configs (`constants/`), computed-value helpers (`lib/`), the server spec (`server-spec.ts`), CRUD handlers + lifecycle hooks + business DAL (`dal/server/` — hooks live in the `createCrudDal` config factory at `dal/server/crud.ts`, never on the spec), action-config hooks (`hooks/`), and reusable components (`components/`). The server spec at `server-spec.ts` is consumed by `src/trpc/routers/proposals.router/`.
...
`../incentives/service.ts:replace`, `contracts.router.ts:applyEnvelopeContext`,
...
Proposals can carry attached files (photos, videos, PDFs) in `proposal_media_files`, owned via the reusable `mediaService`/`MediaStore` seam (see `src/shared/modules/media/DOCS.md`) with `proposalMediaStore`.
...
- **Copy-to-project (manual, agent-driven)**: `projectsRouter.media.importFromProposal` lets an agent import a proposal's **image** files into a linked project's public gallery. The picker (`listImportableProposalMedia`) groups importable files by source proposal with a `Select all` affordance — the agent explicitly chooses which files, not homeowner-visibility-filtered or auto-selected. On import, each file is R2-copied within the same public bucket (`r2Client.copyObject`, `tpr-media` → `tpr-media`; source key `proposals/{proposalId}/{fileId}{ext}` → a fresh dest key `projects/{projectId}/uncategorized/{newId}{ext}`) and inserted as a project media row (`phase: 'uncategorized'`, agent re-organizes later) via `projectMediaCrud.create` (`modules/projects/media/service.ts#importFromProposal`), whose own `create.after` hook dispatches the normal image-optimization job — the same row-lifecycle-as-hook rule as everywhere else in the media module (`docs/codebase-conventions/service-architecture.md#row-lifecycle-vs-orchestration`). The server enforces **image-only** (`mimeType LIKE 'image/%'`) and that the source proposal belongs to a meeting linked to **this** project (`meetings.projectId = input.projectId`) — arbitrary proposal media can't be imported by id. The source proposal's files are left unchanged (copy, not move).
...
**Reference impl**: schema `src/shared/db/schema/proposal-media-files.ts`; router `src/trpc/routers/proposals.router/media.router.ts`; DAL + view projection `src/shared/modules/proposals/media/dal/server/queries.ts` (`toProposalMediaView`); URL derivation `src/shared/lib/get-optimized-urls.ts` (`deriveOriginalMediaUrl`, shared with project media); `getFullView` enrichment `dal/server/queries.ts`; copy-to-project `src/trpc/routers/projects.router/media.router.ts` (`listImportableProposalMedia`, `importFromProposal`)
...
- [`../../media/DOCS.md`](../../media/DOCS.md) — the `mediaService`/`MediaStore` seam `#proposal-media` builds on
