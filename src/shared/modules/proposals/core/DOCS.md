...
**Reference impl**: schema `src/shared/db/schema/proposal-media-files.ts`; router `src/trpc/routers/proposals.router/media.router.ts`; DAL + view projection `src/shared/modules/proposals/media/dal/server/{queries,authz}.ts` (`toProposalMediaView`); URL derivation `src/shared/lib/get-optimized-urls.ts` (`deriveOriginalMediaUrl`, shared with project media); `getFullView` enrichment `dal/server/queries.ts`; copy-to-project `src/trpc/routers/projects.router/media.router.ts` (`listImportableProposalMedia`, `importFromProposal`)
