...
export function monthsForLocale(localeName: Intl.LocalesArgument, monthFormat: Intl.DateTimeFormatOptions['month'] = 'long') {
  const format = new Intl.DateTimeFormat(localeName, { month: monthFormat })
    .format

  return [...Array.from({ length: 12 }).keys()].map(m =>
    format(new Date(Date.UTC(2021, m, 2))),
  )
}

export function daysForLocale(locale: Intl.LocalesArgument, startDay: number) {
...
function OutOfBoundsDay({ day }: OutOfBoundsDayProps) {
  return (
...
export function CalendarBody({ features, children }: CalendarBodyProps) {
  const [month] = useCalendarMonth()
  const [year] = useCalendarYear()
  const { startDay } = use(CalendarContext)
...
export function CalendarMonthPicker({
  ...
  const { locale } = use(CalendarContext)
...
export function CalendarDate({ children }: CalendarDateProps) {
  return <div className="flex items-center justify-between p-3">{children}</div>
}
...
export function CalendarHeader({ className }: CalendarHeaderProps) {
  const { locale, startDay } = use(CalendarContext)
...
export function CalendarProvider({
  ...
  return (
    <CalendarContext value={{ locale, startDay }}>
