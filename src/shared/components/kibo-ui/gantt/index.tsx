'use client'
/* eslint-disable ts/no-use-before-define -- vendored Kibo UI: components reference GanttColumns before its const definition (safe at render time) */
...
function getsDaysIn(range: Range) {
...
function getDifferenceIn(range: Range) {
...
function getInnerDifferenceIn(range: Range) {
...
function getStartOf(range: Range) {
  let fn = startOfDay
...
function getAddRange(range: Range) {
  let fn = addDays
...
function getDateByMousePosition(context: GanttContextProps, mouseX: number) {
...
function createInitialTimelineData(today: Date) {
  const data: TimelineData = []

  const emptyQuarters = () => Array.from({ length: 4 }).fill(null) as TimelineData[number]['quarters']

  data.push(
    { year: today.getFullYear() - 1, quarters: emptyQuarters() },
    { year: today.getFullYear(), quarters: emptyQuarters() },
    { year: today.getFullYear() + 1, quarters: emptyQuarters() },
  )

  for (const yearObj of data) {
    yearObj.quarters = Array.from({ length: 4 }).fill(null).map((_, quarterIndex) => ({
      months: Array.from({ length: 3 }).fill(null).map((_, monthIndex) => {
...
function getOffset(date: Date, timelineStartDate: Date, context: GanttContextProps) {
...
function getWidth(startAt: Date, endAt: Date | null, context: GanttContextProps) {
  const parsedColumnWidth = (context.columnWidth * context.zoom) / 100
...
function calculateInnerOffset(date: Date, range: Range, columnWidth: number) {
    ...
    throttle(() => {
      ...
      if (scrollLeft === 0) {
        ...
        newTimelineData.unshift({
          year: firstYear - 1,
          quarters: Array.from({ length: 4 }).fill(null).map((_, quarterIndex) => ({
            months: Array.from({ length: 3 }).fill(null).map((_, monthIndex) => {
              const month = quarterIndex * 3 + monthIndex
              return {
