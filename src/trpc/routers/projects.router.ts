...
import { createProjectFormSchema, projectFormSchema } from '@/shared/entities/projects/schemas'
...
export const projectsRouter = createTRPCRouter({
  // ── Portfolio (public, presentational projects — no customerId/ownerId) ──

  getPortfolioProjects: baseProcedure
    .query(async () => {
      return getShowroomProjects()
    }),

  getPortfolioProjectDetail: baseProcedure
    .input(z.object({ accessor: z.string() }))
    .query(async ({ input }) => {
      return getShowroomProjectDetail(input.accessor)
    }),

  // ── Agent: portfolio project CRUD ──

  getAllPortfolioProjects: agentProcedure
    .query(async () => {
  ...
  createPortfolioProject: agentProcedure
  ...
  // ── Agent: business project creation (real projects with customer + owner) ──

  createBusinessProject: agentProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      // 4. Link meeting to project and set outcome
      await db
        .update(meetings)
        .set({ projectId: project.id, meetingOutcome: 'converted_to_project' })
        .where(eq(meetings.id, input.meetingId))

      // 5. Extract scope IDs from proposals' SOWs and link to project
