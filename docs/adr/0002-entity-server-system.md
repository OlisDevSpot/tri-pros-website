...
> **AMENDED 2026-08 (CRUD-DAL sub-plans A/D):** hooks and `duplicate` no longer live on the spec — `spec.hooks`/`spec.duplicate` were deleted. They live in the `CrudConfig` returned by the config factory passed to `createCrudDal` (`entities/<entity>/dal/server/crud.ts`), consumed as `<entity>Crud.<handler>`. The spec keeps identity/visibility/schemas/`shareable` only. The paragraphs below describe the original 2026-05 design.
...
- **Domain errors via throw instead of `DalReturn<T>`.** Initially considered: DAL functions throw typed errors (`Error('NotFound')`) and tRPC maps them. Rejected: discriminated union `DalReturn<T>` (WebDevSimplified pattern) is safer — DAL never throws, callers decide error handling. tRPC uses `dalToTrpc()` to map errors; services inspect the union directly. Adopted as the standard.
...
- **Adding a new entity becomes:** one spec file + one constant import in `abilities.ts` + one `createEntityRouter` call. *(Superseded — see the Amendment: `createEntityRouter` was deleted 2026-08-11; today it is a `procedures.ts` + pure-composition `index.ts` pair. `docs/how-to/add-an-entity.md` has the current recipe.)* CRUD works immediately with zero DAL code (generic factory). Business sub-routers use `entity.authedProcedure` for superpowers. See `docs/how-to/add-an-entity.md`.
...
- **Service-layer sub-routers receive the entity toolkit via factory pattern.** Reference implementation: `proposals.router/delivery.router.ts`. A sub-router that orchestrates services (email, notifications, jobs) is declared as `createDeliveryRouter(entity: EntityToolkit<TTable>)` and wired inside the parent `createEntityRouter` factory: `delivery: createDeliveryRouter(entity)`. The sub-router uses `entity.authedProcedure` / `entity.publicProcedure` — it gets scope middleware for free. The procedure body orchestrates: (1) call services (which may call DAL internally), (2) call generic CRUD DAL for updates (`handlers.update(ctx, { id, data })`), (3) call cross-entity DAL for side-effects (with `SYSTEM_CONTEXT`), (4) dispatch async jobs. Services never import `db` — they access data through DAL functions with `SYSTEM_CONTEXT` or a passed-in context. No ad-hoc DAL wrappers for simple field updates; use generic CRUD.

- **`ctx: ScopedContext = SYSTEM_CONTEXT` — the universal DAL calling convention.** Every DAL function accepts `ScopedContext` as its first argument. `SYSTEM_CONTEXT` is a sensible default (full access, no visibility scoping) but callers choose what to pass: tRPC procedures pass their middleware-resolved scoped `ctx`, services accept `ctx` from their caller and thread it to DAL (defaulting to `SYSTEM_CONTEXT` when invoked from jobs/webhooks), and a server-initiated caller acting on behalf of a specific user builds one via `buildUserContext(userId, role, spec)` (`shared/dal/server/lib/helpers.ts`). *(Corrected 2026-09-17: no `buildSessionContext` helper exists in the codebase — this line named one when the ADR was written. RSC pages that read entity data have not yet been wired onto a scoped-context helper at all; today they either use `SYSTEM_CONTEXT` or don't call DAL directly. Wiring RSC callers onto the service layer is tracked in this repo's projects/media modules epic, spec B / R11.)* This replaces the old `ownerKey` pattern (null for omni, userId for scoped) — visibility is now a SQL predicate on `ctx.scope`, not an ad-hoc parameter.
