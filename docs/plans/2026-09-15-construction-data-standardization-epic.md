...
> **Status:** requirements and decisions locked. **P0 shipped 2026-09-20** — all 8 tasks complete (pagination, non-throwing adapters, id normalization, dead-code deletion, energy-trade fix, `providers/notion/DOCS.md` updated); D9 decided (delete the scratch page with `getScopesByTrade`). Next: spec P1.
...
| **P1** | **Seam + read model** — domain schemas, `ConstructionCatalogSource` + `sources/notion/`, provider reduced to `client.ts` + `types.ts`, `constructionService` (cached), `notionRouter`→`constructionRouter`, `buildCatalogIndex`, `useConstructionCatalog`, hooks out of the provider | C1, C6 · F1, F2, F6, F7, F8, F16 · A1, A2, A3, A4, A7 | P0 | not written | — | [ ] |
...
- [ ] **C6** Notion→domain translation, property maps, data-source ids and extractors live in `modules/construction/sources/notion/`; `providers/notion` exposes only `client.ts` + `types.ts`. (D3)
...
- [ ] **F2** A `ConstructionCatalogSource` interface with exactly one binding; the Notion implementation is the only code that knows Notion.
...
- [ ] **F7** `constructionRouter` replaces `notionRouter` in one change, every call site repointed. Dead procedures deleted — **corrected 2026-09-15 against the code:** only `getTradesByQuery` is truly dead (its sole caller is the dead `useGetTrades` hook). `getScopesByTrade` has one consumer, the scratch page `app/(frontend)/(site)/tests/[label]/page.tsx:17`, which goes with it. **`getScopesByQuery` is LIVE** — `useGetScopes` feeds `sow-field.tsx`, `shared/components/trade-scope-row.tsx` and `meeting-scopes-picker.tsx`; it can only be deleted at **P3**, when the shared `CatalogIndex` replaces per-row scope queries.
- [ ] **F8** `useConstructionCatalog()` + `buildCatalogIndex` is the only client read of the catalog; no per-trade-row scope queries; RSC consumers call the service directly.
...
- [x] **A3** Adapter behavior — `providers/notion/DOCS.md#adapter-returns-entity-or-null`, `#disabled-checkbox-is-extraction-time-gate`, `#notion-select-is-source-of-truth-for-zod-enums`, `#cache-invalidation-after-notion-edits`. Update that DOCS with F3–F6.
- [ ] **A4** tRPC: routers are named for a real entity/domain, never a vendor — `src/trpc/DOCS.md#procedures-defined-once`, `#one-leaf-shape`, `#pure-composition-index`, `#dal-to-trpc-bridge`.
- [ ] **A5** Enums — `enum-standardization.md#const-array-source-of-truth`, `#type-derived-from-const`, `#text-with-enum`. The category/kind/pricing-unit arrays are the single source; the Notion Zod enums import them.
...
| **B18** | **`disabled` exists only on trades.** Scopes, SOWs and pain points have no `disabled` property or schema field, so F4's "drop disabled rows at extraction" applies to trades alone today. `use-trade-catalog.ts:16` re-filters `disabled` client-side — the anti-pattern `notion/DOCS.md:72` names. | P0 ✅ (note, recorded in `notion/DOCS.md`) / P1 (remove the re-filter) |
...
- **Provider:** `src/shared/services/providers/notion/**` — client, generic query DAL, 4 adapters, and React Query hooks in `dal/**/hooks` (a provider→tRPC dependency inversion).
- **Service:** `src/shared/services/construction-data.service.ts` — 7 functions, returns Notion types.
- **Router:** `src/trpc/routers/notion.router/{index,trades,scopes}.router.ts` — public, uncached.
- **Cache bypasses:** `features/landing/lib/notion-trade-helpers.ts` (`unstable_cache`, 180s) · `features/meeting-flow/lib/get-cached-pain-points.ts` (feature→provider DAL, 600s) · `trpc/routers/customers.router/business.router.ts:185-191` (uncached id→name) · `scripts/portfolio-scraper/fetch-scopes.ts` (a second Notion client).
- **Enums:** `src/shared/domains/construction/constants/enums.ts` — outside `constants/enums/`, folds into the module.
...
- [ ] **K3** `providers/notion/DOCS.md` — rewrite for F3–F6 at P1; correct the database list and the 180s-vs-600s TTL split.
- [ ] **K4** Create `src/shared/modules/construction/DOCS.md` with the A8 anchors (P1 skeleton, P2 rules).
