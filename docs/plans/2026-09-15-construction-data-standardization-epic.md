...
> **P1 shipped 2026-09-22** — 9 tasks, commits `616465a7`..`086148e2` plus this docs commit. The module, the seam, the cached service, `constructionRouter`, the shared read model and the scraper's move onto the seam all landed; see §3 for the per-requirement state and the amendments below. **Owner escalations:** P0 #1 (public uncached catalog reads) and #4 (`sortBy` admitting relation properties) **close with P1**; #2 (IRA 25C claim accuracy in `programs.ts:22,113,139`) and #3 (`program-step.tsx`'s missing loading gate) stay **open and owner-owned** — **#2 mechanism confirmed 2026-09-22** against live Notion: Roof & Gutters, Dryscaping, Exterior Paint and Water heating all carry `Energy Efficiency`, so `qualifyEnergySaver` qualifies on them and `program-card.tsx:88-92` renders the `$1,200` 25C line unconditionally; evidence in the P2 spec Appendix C. **Next: P2, then P3 — per the owner's 2026-09-22 directive (C7), features stop re-implementing catalog logic and consume the module's primitives instead.**
...
| **P2** | **Rules + identity** — taxonomy/energy/primary-trade consolidated + `DOCS.md`; Notion `Slug` property, backfill, required-slug adapter; slug-key validation; `TRADE_FACTS` name drift | C2, C5, C7 (rules half) · F9, F10, F11, F12, F17, F18 · A5, A8, A11, A12 · B5, B6, B20 | P1 | [spec](../superpowers/specs/2026-09-22-construction-p2-rules-and-identity-design.md) — brainstormed 2026-09-22, awaiting owner review | **two plans** (spec §7): plan 1 = external writes (Notion `Slug` + backfill, snapshot, `pricingUnits`, taxonomy DDL), plan 2 = pure code — not written | [ ] |
...
**Spec-time details** (decided inside the spec that owns them, not here): P1 — cache TTL and tag names, service method names, router leaf names, whether `getCatalog` splits trades/scopes. P2 — **decided 2026-09-22 in the P2 spec §2:** `Slug` is a Notion `rich_text` property; the backfill is dry-run by default with an owner-run `--apply`; the slug-key check is a generated `TradeSlug` snapshot enforced by `tsc` plus `scripts/verify-catalog-keys.ts` for id-keyed maps (retires at P5). P3 — the `TradeScopeRow` value-adapter prop shape. P4 — `_v` bumps per entity schema.
...
| **D4** | **Closed — canonical pairing + outcome copy.** One code registry in the module (`core/constants/trade-pairings.ts`, `trade-outcomes.ts`), slug-keyed, typed by the `TradeSlug` snapshot. **Two registers per entry, named for the surface (owner, 2026-09-22: "`landing-copy` and `presentation-copy` for now, to be explicit"; code keys `landingCopy` / `presentationCopy`):** `presentationCopy` (in-home meeting presentation, ⇐ `docs/sales/in-home-meeting-playbook.md`; for pairings it is the **agent-only** panel cue, for outcomes the homeowner-visible stage lead) and `landingCopy` (public services pages, ⇐ the shipped landing copy, itself written from `docs/proposal/scope-presentation.md`). `landingCopy` required, `presentationCopy` optional with fallback to `landingCopy` through one helper (`registerCopy`). Pairings are trade-level, **directed**, many per trade; the first entry is the meeting card's pick (`primaryPairing`). Paired *names* resolve from the catalog, never the registry. These names supersede design §6's `story`/`reason` and the brainstorm's interim `web`/`spoken`. The roof/HVAC "disagreement" was two registers, not a conflict. Copy is verbatim in the P2 spec Appendix A. | 2026-09-22 |
| **D-f…D-m** | **P2 shape decisions** (P2 spec §2). **D-f:** F12 = generated `TradeSlug` snapshot enforced by `tsc` + `scripts/verify-catalog-keys.ts`; **retires at P5** when slugs are a column. **D-g:** rules move and their old homes die; UI edits are import-level plus the five `useMemo`-body swaps the spec names; picker-row internals stay P3. **D-h:** `constructionTypes` is retired, `tradeCategories` is the one taxonomy, one DDL drops `scopes.construction_type` + the enum; **P5 note:** taxonomy returns to Neon as an enum mirroring `tradeCategories`. **D-i:** lead trade names resolve in one place, `customerIntakeService.ingestLead`, from the cached catalog; the funnel page resolves its display name at the RSC; the Bina normalizer emits funnel slugs, not names; GHL free text stays raw. **D-j:** F9's resolver ships in the module and every non-component site plus the five named component sites repoint; sites that fall back to a *persisted* label snapshot are P4 (A9). **D-k:** F10 was delivered by P0 + P1; P2 only writes `#scope-vs-addon`. **D-l:** `src/pain-points.ts` stays until P4, which extracts anything worth keeping before deleting (see D8). **D-m:** one spec, **two plans** — plan 1 external writes, plan 2 pure code. | 2026-09-22 |

## 2. Open decisions

| ID | Question | Needed by | Recommendation |
|---|---|---|---|
| ~~**D4**~~ | **Closed 2026-09-22 → §1.** | — | — |
...
| **D8** | **Pain points** — fold into the module at P4, or leave to #195's deferred follow-up? Five vocabularies exist today. | Before P4 | **P4.** They already flow through the same Notion path and the same 100-row/throwing-adapter hazards. **Added 2026-09-22 (P2 brainstorm, D-l):** `src/pain-points.ts` (1,313 lines, zero importers) is one of the five; it stays until P4, which **extracts anything worth keeping before deleting it** — owner: "that file may contain a lot of useful painPoint data and structure". |
...
- [ ] **C7** **Features consume the module's primitives; they never re-implement them.** A feature may hold presentation and choreography. Catalog shapes, rules, resolvers, classifiers and reads come from `modules/construction` — `useConstructionCatalog`/`buildCatalogIndex` for reads, the module's schemas for types, the module's rules for behaviour. A new trade/scope helper inside `features/` is a defect, not a shortcut. **Owner directive, 2026-09-22**, after P1 landed the home: measured that day, 30+ files under `features/` still carry trade/scope logic (26 in meeting-flow, the rest in landing) while only ~32 files import the module. **Corrected 2026-09-22 (P2 brainstorm):** 52 files outside the module import it, 20 by value (`constructionService` ×5, `useConstructionCatalog` ×2, `buildCatalogIndex` ×1, `tradeCategories` ×1, scripts' `catalogSource` and adapters) and the rest `import type` — features take the module's *types* and re-implement its *rules*, which is exactly what this directive reverses. **P2 owns the rules (F9–F12, F17, F18); P3 owns the UI (F13, A6, A7) and F8's three remaining per-row queries.**
...
- [ ] **F9** One trade/scope id→label resolver (replaces 9). Unknown ids surface as orphans, never silently dropped. **Amended 2026-09-22 (P2 spec §4.5, §4.7-C, Appendix D):** the resolver is `core/lib/resolve-catalog-ids.ts` (`resolveTrades`/`resolveScopes` → `{ found, orphans }`) plus `scopesById` on the index. A **tenth** site found in the brainstorm: `customers.router/business.router.ts:186-190` builds its own `nameById` from the catalog and silently drops unknown ids — it is also where F18 wants resolution to live, so the two collapse. Sites that fall back to a *persisted* label snapshot (`trade-selection-provider.tsx:132`, `project-row.tsx:45`) are **P4** (A9), cite-only at P2. B8/B10/B11/B19 stay P3. **Partly delivered 2026-09-24 by the portfolio step plan** (`b17f7ae3`, `docs/superpowers/plans/2026-09-23-portfolio-step-project-story.md` Task 5): `scopesById`, `core/lib/resolve-catalog-ids.ts` (`resolveTrades`, `resolveScopes`, `ResolvedCatalogIds<T>`), and the `index-showcase-projects` / `use-showcase-projects` site. Every other F9 site stays P2 plan 2's.
- [x] **F10** One scope-vs-add-on classifier: `kind` set at the adapter (replaces 3 disagreeing classifiers). **Delivered by P0 + P1, recorded 2026-09-22 (D-k):** `kind` is set once at the adapter, `buildCatalogIndex` splits on it, and the only classifier left outside the module is landing's `scopes-grid.tsx:24-25` filtering on `kind`. P2 writes the `#scope-vs-addon` anchor; nothing moves.
...
- [ ] **A5** Enums — `enum-standardization.md#const-array-source-of-truth`, `#type-derived-from-const`, `#text-with-enum`. The category/kind/pricing-unit arrays are the single source; the Notion Zod enums import them. **Corrected 2026-09-22 (P1):** none of those arrays existed — `entryType` and `unitOfPricing` were bare `z.string()`. P1 creates `tradeCategories` and `scopeKinds` in `core/schemas/index.ts` (and deletes the duplicate `TRADE_CATEGORY_ORDER`); **`unitOfPricing` is still a bare string and remains A5's work at P2**, alongside the `constructionTypes`/`tradeCategories` drift recorded at `modules/construction/DOCS.md#category-taxonomy`. **P2 spec (2026-09-22):** `pricingUnits = ['unit','sqft','space','linear ft','bsq']` in `core/schemas/index.ts`, mirrored from the live select (48/23/32/12/5 scopes in use), `unitOfPricing` becomes `z.enum` with no default after a blank-row check; `constructionTypes` is retired (D-h). The seam script asserts both Notion select option lists equal the arrays.
...
| **B19** | **Landing matches portfolio projects to a trade by free-text substring.** `landing/ui/components/services/portfolio-proof.tsx:13-28` lowercases `tradeName` and scores projects by `hoRequirements` strings that `.includes()` it — same class as B10; a Notion rename or a name like "Windows & doors" vs "windows" silently changes the proof strip. Found 2026-09-22 (P2 brainstorm). | P3 |
| **B20** | **`solar` is not in the live catalog** (27 trades read 2026-09-22; the one disabled row is almost certainly Solar). Landing keys pairings (`trade-pairings.ts:17,27-29`), an outcome (`trade-outcome-statements.ts:6`) and the energy pillar's featured roof↔solar pair (`pillar-config.ts:53`) on it, and the trade page links to `/services/energy-efficient-construction/solar`, which 404s. Once registries are typed by the `TradeSlug` snapshot these cannot compile. **Owner call, P2 spec §9:** (a) re-enable Solar, or (b) drop the entries (kept in the spec's Appendix A) and feature roof↔attic instead. Spec assumes (b). | P2 |
