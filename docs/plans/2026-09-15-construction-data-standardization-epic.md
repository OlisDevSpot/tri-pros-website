...
| **D4** | **Closed — canonical pairing + outcome copy.** One code registry in the module (`core/constants/trade-pairings.ts`, `trade-outcomes.ts`), slug-keyed, typed by the `TradeSlug` snapshot. **Two registers per entry:** `spoken` (in-home meeting, ⇐ `docs/sales/in-home-meeting-playbook.md`) and `web` (landing, ⇐ the shipped landing copy, itself written from `docs/proposal/scope-presentation.md`). `web` required, `spoken` optional with fallback to `web` through one helper (`registerCopy`). Pairings are trade-level, **directed**, many per trade; the first entry is the meeting card's pick (`primaryPairing`). Paired *names* resolve from the catalog, never the registry. Field names `web`/`spoken` supersede design §6's `story`/`reason`. The roof/HVAC "disagreement" was two registers, not a conflict. Copy is verbatim in the P2 spec Appendix A. | 2026-09-22 |
