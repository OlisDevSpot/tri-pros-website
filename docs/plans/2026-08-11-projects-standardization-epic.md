...
- [ ] **Phase 2 · HITL · blocked-by: Phase 1 — Case study: `createCrudDal` mutation-interface requirements.**
  Catalog every tagged bypass — `business.create` (proposal-gated + cross-entity meeting update + scope extraction), portfolio `crud.create` (hard-set scopes), `x_project_scopes` replace, `media.*` (movePhase / toggleHero / importFromProposal), `accounting.service` project write. For each: **why** it bypasses `createCrudDal` today, and the **interface capability** that would let it route through (candidates: pre-insert derived-field enrichment, cross-entity orchestration inside a hook, per-call permission variance, multi-row transactional replace, business-precondition gating). Produce the **mutation-interface extension requirements** and hand to `project-crud-mutation-standardization`'s session.
  - **Plan:** _(JIT)_ · **HITL:** confirm the requirement set before hand-off.
  - **AC:** every projects mutation bypass catalogued with its interface-gap rationale; a requirements deliverable exists and is linked from `project-crud-mutation-standardization`.
