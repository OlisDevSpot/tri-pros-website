...
**Partly resolved 2026-09-24 (`403b29d5`):** `steps/portfolio-step.tsx` and `steps/trade-project-grid.tsx` are deleted; step 3 reads through `usePortfolioProjects` (modules/projects/core) and `matchPortfolioProjects`. Three cross-feature imports remain, in two files: `ui/components/table/index.tsx` (2) and `lib/to-calendar-event.ts` (1).
...
**Partly resolved 2026-09-24 (`403b29d5`):** the first two items are gone with `portfolio-step.tsx`. Step 3 no longer filters or paginates the portfolio on the client; it ranks the list once with `matchPortfolioProjects` over `usePortfolioProjects`.
