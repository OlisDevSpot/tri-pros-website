...
- **~~Bug fix across schema files.~~** ⚠️ **REVERSED 2026-06-02.** The grill claimed `agentUserId: text(...)references(user.id)` was a bug ("text referencing a uuid PK") and "corrected" to `uuid`. This was wrong — `user.id` is `text` (better-auth uses string IDs like nanoid/cuid, not uuid). Every existing FK to `user.id` in the codebase uses `text` (activities, meetings, projects, push-subscriptions, customer-notes, etc.). The "fix" introduced a real bug that surfaced at `pnpm db:push:dev` time: `foreign key constraint cannot be implemented … Key columns "assigned_user_id" and "id" are of incompatible types: uuid and text`. **Correct rule: any FK to `user.id` MUST be `text`, not `uuid`.** All voip-in-house schema files (voip_dids, voip_calls, voip_messages, voip_link_tokens, app_settings) + `customers.dncAddedByUserId` have been corrected back to `text`.
