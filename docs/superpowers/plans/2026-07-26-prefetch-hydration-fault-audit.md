...
Wave 3 landed 2026-07-26 (`2026-07-26-wave3-prefetch-conversions.md`): Task 1 meetings `6b021526` · Task 2 projects `040a9946` · Task 3 proposals `fd9e705f` · Task 4 schedule `27313ac6` · Task 6 campaigns-leads config split `a08f8641` · Task 7 lead-sources configs `119c782a` · Task 8 (this commit) flips the eslint rule to error. Scope was rule-trimmed before execution, per that plan's per-screen adjudication table: only meetings/projects/proposals/schedule got the server-prefetch conversion; settings (Task 5) and the lead-sources prefetch (Task 9) were DROPPED (fail the when-to-prefetch rule's condition 3, and condition 2 for lead-sources); campaigns-leads (Task 6) and lead-sources (Task 7) got config-extraction ONLY and stay client-fetched. Do not read this as "Tasks 1-8 all converted" — two of the nine tasks are config-only refactors with no prefetch, and two were dropped outright.

Action-center dropped from scope: `ActionCenterView` has no route — it's a sheet mounted from the sidebar (`app-sidebar.tsx:300`), not a `page.tsx` — so the "settings / action-center" conversion recipe above assumed a route that doesn't exist. There is nothing to prefetch from; no follow-up is needed.

Precision fix to guardrail 5's framing and the "Accepted as convention" line above: dynamic `FilterDefinition.options` are key-irrelevant ONLY when the shared config declares `options: []` — empty options parse as free strings identically on both sides (`derive-paginated-query-state.ts:60`). Non-empty options ARE parser-relevant (`parseAsStringLiteral` validation applies) since Wave 1 P1's isomorphic option validation landed. The blanket "dynamic options are key-irrelevant" statement was accurate only for the split-config pattern (campaigns-leads, Task 6) where the hook always receives `options: []`; it does not generalize to configs with populated options.
