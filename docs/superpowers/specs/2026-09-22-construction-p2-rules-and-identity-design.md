...
| ID | Decision |
|---|---|
| **D4 — closed** | One code registry in the module, slug-keyed. Two registers per entry: `spoken` (in-home meeting, from `docs/sales/in-home-meeting-playbook.md`) and `web` (landing, from the live landing copy, which was written from `docs/proposal/scope-presentation.md`). `web` is required, `spoken` is optional and falls back to `web`. Pairings are trade-level, directed, many per trade; the first entry is the meeting-flow card's pick. Paired names come from the catalog, never the registry. Field names are `web` and `spoken`, superseding design §6's `story` and `reason`. |
...
| `core/constants/trade-pairings.ts` | `TradePairing { pairedSlug: TradeSlug; web: string; spoken?: string }` and `TRADE_PAIRINGS: Partial<Record<TradeSlug, readonly TradePairing[]>>`. | D4, C5 |
| `core/constants/trade-outcomes.ts` | `TradeOutcome { web: string; spoken?: string }` and `TRADE_OUTCOMES: Partial<Record<TradeSlug, TradeOutcome>>`. | D4, C5 |
...
| `core/lib/register-copy.ts` | `registerCopy(entry, register: 'web' \| 'spoken'): string \| undefined` — the one place `spoken ?? web` lives. | D4 |
...
- `web` is required on every entry; `spoken` is optional. Consumers read copy only through `registerCopy(entry, register)`.
...
- `showcase-text.tsx:26` → `registerCopy(TRADE_OUTCOMES[trade.slug], 'spoken')`; `:27` passes `trade.slug` to `selectTradeBenefits`.
...
Landing:
- `trade-card.tsx:52` → `registerCopy(TRADE_OUTCOMES[trade.slug], 'web') ?? <existing fallback sentence>`. The fallback stays: it is presentation for a trade with no entry.
- `trade-view.tsx:42` and `pillar-view.tsx:32` → `resolveTradePairings(slug, index)` / `resolveFeaturedPairings(pillarSlug, index)` in `src/features/landing/lib/resolve-trade-pairings.ts`, returning `{ trade: Trade; pillarSlug: PillarSlug; web: string }[]`. A paired slug the live catalog lacks is `console.warn`ed and skipped, never silent. The index comes from the same `constructionService.getCatalog()` read the page already does; B11's double computation is P3.
...
Sources: **playbook** = `docs/sales/in-home-meeting-playbook.md` (`:180-189` outcomes, `:191-196` pairings). **landing** = the shipped landing constants, which were written from `docs/proposal/scope-presentation.md:34-52` (outcomes) and `:56-69` (pairings); where landing and the doc differ by a word, landing wins because it is the web register as shipped. **meeting** = the shipped meeting-flow constants, which were written from the playbook.

### A.1 `TRADE_OUTCOMES`

| slug | `web` (landing `trade-outcome-statements.ts`) | `spoken` (playbook / meeting `trade-outcomes.ts`) |
|---|---|---|
...
**Dropped, not live:** `electricals` (landing `:22` — the live slugs are `electricals-finish` / `electricals-rough`, which have no copy; V5 warns) and `solar` (§9 option b: web "You're locking in your energy rate for 25 years while your neighbors keep paying whatever the utility charges."; spoken "You're essentially locking in your energy rate for 25 years — while your neighbors keep seeing their bills go up."). **No copy yet:** `demo-and-hauling`, `electricals-finish`, `electricals-rough`.

### A.2 `TRADE_PAIRINGS` (directed; first entry = meeting card)

| from | to | `web` (landing `trade-pairings.ts`) | `spoken` (playbook / meeting) |
|---|---|---|---|
...
Order within a trade is the landing order, which already puts the meeting card's pick first for every trade that has a spoken entry. `bathroom-remodel → flooring` is new to meeting-flow (the playbook has it, meeting-flow did not — the D4 gap the tracker noted).

**Dropped, not live (§9):** `roof-and-gutters → solar` (web "A new roof is the ideal foundation for solar — one installation sequence, better ROI.") and `solar → roof-and-gutters` (web "A new roof is the ideal foundation for solar — one install, better ROI.").
