...
| File | Role |
|------|------|
| `src/trpc/lib/create-entity-router.ts` | Factory — builds toolkit, calls factory fn. Exports `EntityToolkit<TTable>` |
| `src/trpc/lib/create-crud-router.ts` | Maps CRUD slots to tRPC procedures (unused by proposals, kept for simple entities) |
| `src/trpc/lib/middleware/scope-middleware.ts` | Resolves `ctx.scope` from CASL + visibility predicate |
| `src/trpc/lib/middleware/shareable-middleware.ts` | Token-or-session auth with scope resolution |
...
| File | Role |
|------|------|
| `src/shared/dal/server/lib/types.ts` | Canonical types: `ScopedContext`, `DalReturn<T>`, `DalError`, `EntityServerSpec`, `CrudHandlers`, `SYSTEM_CONTEXT` |
...
1. **`entity.crud()` type inference**: The `createCrudRouter` factory loses tRPC type inference due to dynamic procedure construction. Proposals inlines CRUD for full types. Fix the factory or accept inline as pattern for complex entities?

2. **Cross-entity writes in contracts router**: `contracts.router.ts` writes to customers table from proposals router. Should call customer DAL functions once customer is standardized.

3. **`buildSessionContext` for RSC**: Currently calls `auth.api.getSession({ headers: new Headers() })`. May not work in all RSC contexts. Validate when first RSC consumer is written.

4. **JSONB merge on update**: Declared on spec (`jsonbMergeColumns`) but NOT implemented in generic DAL. When a consumer needs JSONB merge (proposal form saves), implement in generic DAL or as a proposal-specific update override?

5. **Visibility behavior change**: Old system scoped proposals by `ownerId`. New system scopes by `userParticipatesInMeeting`. Intentional improvement (fixes visibility leak) but may surface in testing.

6. **Contracts router architecture**: Is it truly a proposals sub-router, or should it be its own service-domain router? It orchestrates Zoho Sign + reads proposals + writes customers + dispatches jobs. Brainstorm before migrating.
